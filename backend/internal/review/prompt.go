...
1. For each PR below, post a separate review on that pull request and capture its id in one call. Post with `+"`gh api`"+` rather than `+"`gh pr review`"+`: it is the only way to attach inline comments, and its response carries the created review's id, so AO can tell the worker exactly which review to address. Send the review as a JSON body so the inline comments form a proper array of objects:
    ...
    printf '%%s' '{ "event": "COMMENT", "body": "<summary>\n\n<ao-review-marker-from-queue>", "comments": [ { "path": "<file>", "line": <n>, "body": "<finding>" } ] }' | gh api --method POST repos/{owner}/{repo}/pulls/{number}/reviews --input - --jq '.id'
   ...
   - Replace <ao-review-marker-from-queue> with the exact marker shown for that task, replacing only its verdict placeholder with approved or changes_requested. Keep the marker as the final line of the GitHub review body.
...
Do not include the ao-review marker in the JSON sent to `+"`ao review submit`"+`; its body is the human-readable review markdown only.
...
func reviewQueueText(spec LaunchSpec) string {
	if len(spec.ReviewQueue) <= 1 {
		return fmt.Sprintf("\nReview task queue:\n* 1. %s (head commit %s, run %s)\n  Provider marker: <!-- ao-review:v1 run=%s sha=%s verdict=<approved|changes_requested> -->\n", spec.PRURL, spec.TargetSHA, spec.RunID, spec.RunID, spec.TargetSHA)
	}
	...
	for i, task := range spec.ReviewQueue {
		fmt.Fprintf(&b, "* %d. %s (head commit %s, run %s)\n  Provider marker: <!-- ao-review:v1 run=%s sha=%s verdict=<approved|changes_requested> -->\n", i+1, task.PRURL, task.TargetSHA, task.RunID, task.RunID, task.TargetSHA)
	}
	return b.String()
}
